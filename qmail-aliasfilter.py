...
def main():
    ...
    #print msg
    #print "Redirect into maildir: " + str(redirect_spam_into_maildir)

    try:
        default_alias = os.getenv('DEFAULT')
        #print "Default alias before: " + str(default_alias)
        ...
        # which matches the current recipient address.
            ...
            #default_alias = os.getenv('ALIAS')
            #default_alias = msg['X-qmail-default']
            #default_alias = "nospam"
            #del msg['X-qmail-default']
        #print "Default alias after: " + str(default_alias)
        
    except:
        default_alias = '$DEFAULT_not_specified'
        #print "Default alias exception: " + str(default_alias)
    ...
    logToFile('default_alias: ' + default_alias)
    logToFile('sender_address: ' + sender_address)
    logToFile('recipient_address: ' + recipient_address)
    ...
    while i <= str.count(default_alias, '+'):
        ...
        if len(str.strip(current_alias[i], '.')) == 0:
            ...
            if sender_hostname[1] == current_alias[i]:
                ...
                # nothing more to do
                logToFile('Match for case (1): nospam-example.com@... -> accepts all incoming mail from example.com, nothing more to do')
                ...
                if wildcard_alias == current_alias[i]:
                    match_found = match_found+1	# Yeah! We found a match.
                    logToFile('Match for case (2): nospam-.example.com@... -> accepts all incoming mail from *.example.com')
                ...
                if wildcard_alias == current_alias[i]:
                    match_found = match_found+1	# Yeah! We found a match.
                    logToFile('Match for case (3): nospam-example.@... -> accepts all incoming mail from example.*')
                ...
                if wildcard_alias == current_alias[i]:
                    match_found = match_found+1	# Yeah! We found a match.
                    logToFile('Match for case (4): nospam-.example.@... -> accepts all incoming mail from *.example.*')

        i = i+1

    if match_found >= 1:
        # the domain part of the senders email address matches the $DEFAULT alias of qmail
        logToFile('[Accepted] qmail-aliasfilter accepted an email from \'%s\' to \'%s\'' %(sender_address, recipient_address))
        ...
        logToFile('[Rejected] qmail-aliasfilter rejected an email from \'%s\' to \'%s\'' %(sender_address, recipient_address))
...
def logToFile(message):
    homedir = os.getenv('HOME')
    logfile = open('%s/qmail-aliasfilter.log' %homedir, 'a')
    print >> logfile, '[%s] %s' %(time.asctime(), message)
    logfile.close()
